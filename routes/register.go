...
func create_user(database *mongo.Client) gin.HandlerFunc {
	fn := func(ctx *gin.Context) {
		...
		check, err := collection.CountDocuments(ctx, bson.M{"email": to_insert.Email})
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err})
			return
		}
		if check > 0 {
			ctx.JSON(http.StatusUnauthorized, gin.H{"error": "this email is already used"})
			return
		}
		db_response, err := collection.InsertOne(ctx, to_insert)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err})
			return
		}
